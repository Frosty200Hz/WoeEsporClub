...
module.exports.run = async (bot, message, args, member, client, level) => {
  ...
  if(!isim)
    return message.channel.send(new Discord.MessageEmbed()
    .setDescription(`Bir Nick Gir Örnek: \`${ayarlar.prefix}kadın @Kullanıcı <nick> <isim>\``)
    .setColor("RED"));

  const isim2 = args[2];  // Yeni parametre 'isim2' burada alınıyor.
  if(!isim2)
    return message.channel.send(new Discord.MessageEmbed()
    .setDescription(`Bir İsim Gir Örnek: \`${ayarlar.prefix}kadın @Kullanıcı <nick> <isim>>\``)
    .setColor("RED"));

  const yaş = args[3];  // Yaş artık args[3] olacak
  if(!yaş)
    return message.channel.send(new Discord.MessageEmbed()
    .setDescription(`Bir Yaş Gir Örnek: \`${ayarlar.prefix}kadın @Kullanıcı <nick> <isim>\``)
    .setColor("RED"));
...
sohbetKanal.send(embed01);
  ...
  await user.roles.add('1266868745859502117');
...
exports.help = {
  name: 'kadın',
  description: 'Kayıt',
  usage: 'kadın <@Kişi> <İsim> <İsim2> <Yaş>'
};
